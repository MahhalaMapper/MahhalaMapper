# MahhalaMapper.Extensions.EnumMapping

The built-in enum mapper is not configurable, it can only be replaced. Alternatively, MahhalaMapper supports convention based mapping of enum values in a separate package [MahhalaMapper.Extensions.EnumMapping](https://www.nuget.org/packages/MahhalaMapper.Extensions.EnumMapping/).
...
```c#
using MahhalaMapper.Extensions.EnumMapping;
...
[MahhalaMapper](https://www.nuget.org/packages/MahhalaMapper/) provides a nice tooling for validating typemaps. This package adds an extra `EnumMapperConfigurationExpressionExtensions.EnableEnumMappingValidation` extension method to extend the existing `AssertConfigurationIsValid()` method to validate also the enum mappings.
