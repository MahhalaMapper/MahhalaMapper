...
Starting with 6.1.0, MahhalaMapper now supports richer reverse mapping support. Given our entities:
...
By calling `ReverseMap`, MahhalaMapper creates a reverse mapping configuration that includes unflattening:
...
MahhalaMapper will automatically reverse map "Customer.Name" from "CustomerName" based on the original flattening. If you use MapFrom, MahhalaMapper will attempt to reverse the map:
...
As long as the `MapFrom` path are member accessors, MahhalaMapper will unflatten from the same path (`CustomerName` => `Customer.Name`).
