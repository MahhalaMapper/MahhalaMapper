namespace MahhalaMapper.UnitTests.MappingExceptions;
...
public class When_encountering_a_member_mapping_problem_during_mapping : NonValidatingSpecBase
{
    ...
    public void Should_provide_a_contextual_exception()
    {
        ...
        typeof(MahhalaMapperMappingException).ShouldBeThrownBy(() => Mapper.Map<Source, Dest>(source));
    }
    ...
    {
        var source = new Source { Value = "adsf" };
        MahhalaMapperMappingException thrown = null;
        try
        {
            Mapper.Map<Source, Dest>(source);
        }
        catch (MahhalaMapperMappingException ex)
        {
            thrown = ex;
        }
