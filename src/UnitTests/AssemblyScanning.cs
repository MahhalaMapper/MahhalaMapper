namespace MahhalaMapper.UnitTests
{
    namespace AssemblyScanning
    {
        ...
        {
            private static readonly Assembly MahhalaMapperAssembly = typeof(When_scanning_by_name).Assembly;

            protected override MapperConfiguration CreateConfiguration() => new(cfg =>
            {
                AppDomain.CurrentDomain.AssemblyResolve += OnAssemblyResolve;
                cfg.AddMaps(new[] { MahhalaMapperAssembly.FullName });
                AppDomain.CurrentDomain.AssemblyResolve -= OnAssemblyResolve;
            });

            private static Assembly OnAssemblyResolve(object sender, ResolveEventArgs args) => args.Name == MahhalaMapperAssembly.FullName ? MahhalaMapperAssembly : null;
