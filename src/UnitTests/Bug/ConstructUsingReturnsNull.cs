...
public class ConstructUsingReturnsNull : MahhalaMapperSpecBase
{
    class Source
    {
    ...
    public void Should_throw_when_construct_using_returns_null()
    {
        new Action(() => Mapper.Map<Source, Destination>(new Source()))
            .ShouldThrowException<MahhalaMapperMappingException>(ex=>ex.InnerException.ShouldBeOfType<NullReferenceException>());
    }
}