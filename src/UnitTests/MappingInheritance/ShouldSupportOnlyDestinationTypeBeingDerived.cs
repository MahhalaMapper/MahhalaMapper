...
public class AsWithMissingMap : NonValidatingSpecBase
{
    ...
    public void Should_report_missing_map() => new Action(AssertConfigurationIsValid).ShouldThrow<InvalidOperationException>().Message.ShouldBe(
        "Missing map from MahhalaMapper.UnitTests.MappingInheritance.AsWithMissingMap+TModel to MahhalaMapper.UnitTests.MappingInheritance.AsWithMissingMap+TConcrete. Create using CreateMap<TModel, TConcrete>.");
}
public class AsShouldWorkOnlyWithDerivedTypesWithGenerics : MahhalaMapperSpecBase
{
    class Source<T>
    {
    }
...
public class AsWithGenerics : MahhalaMapperSpecBase
{
