...
namespace MahhalaMapper;

/// <summary>
/// Wraps mapping exceptions. Check exception.ToString() for the full error message.
/// </summary>
public class MahhalaMapperMappingException : Exception
{
    private readonly string _message;

    public MahhalaMapperMappingException()
    {
    }

    public MahhalaMapperMappingException(string message)
        : base(message) => _message = message;

    public MahhalaMapperMappingException(string message, Exception innerException)
        : base(message, innerException) => _message = message;

    public MahhalaMapperMappingException(string message, Exception innerException, TypePair types)
        : this(message, innerException) => Types = types;

    public MahhalaMapperMappingException(string message, Exception innerException, TypeMap typeMap)
        : this(message, innerException, typeMap.Types) => TypeMap = typeMap;

    public MahhalaMapperMappingException(string message, Exception innerException, MemberMap memberMap)
        ...
        get
        {
            return string.Join(Environment.NewLine,
                base.StackTrace
                    ...
                    .Where(str => !str.TrimStart().StartsWith("at MahhalaMapper.")));
        }
    }
#endif
}
...
public class MahhalaMapperConfigurationException : Exception
{
    ...
    public MahhalaMapperConfigurationException(string message)
        : base(message)
    {
    }

    public MahhalaMapperConfigurationException(string message, Exception inner)
        : base(message, inner)
    {
    }

    public MahhalaMapperConfigurationException(TypeMapConfigErrors[] errors) => Errors = errors;

    public MahhalaMapperConfigurationException(TypePair types) => Types = types;
    ...
    public override string Message
    {
        get
        {
            if (Types.HasValue)
            {
                ...
                while (exToUse != null)
                {
                    if (exToUse is MahhalaMapperConfigurationException configExc)
                    {
